...
<section class="content-header">
  <h1>
    <i class="fa fa-star icon-title"></i> Nuestros Trabajos

    <a class="btn btn-primary btn-social pull-right" href="?module=form_services&form=add">
  ...
  <div class="row">
    ...
    elseif ($_GET['alert'] == 1) {
      echo "<div class='alert alert-success alert-dismissable'>
              ...
              El nuevo servicios se ha guardado con éxito.
            </div>";
    }
    ...
    elseif ($_GET['alert'] == 2) {
      echo "<div class='alert alert-success alert-dismissable'>
              <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
              <h4>  <i class='icon fa fa-check-circle'></i> Bien hecho</h4>
              El servicios ha sido actualizado exitosamente.
            </div>";
    }
    ...
    elseif ($_GET['alert'] == 3) {
      echo "<div class='alert alert-success alert-dismissable'>
              <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
              <h4>  <i class='icon fa fa-check-circle'></i> Bien hecho!</h4>
              El servicios ha sido eliminado exitosamente.
            </div>";
    }
    ...
    ?>
      ...
      <div class="box box-primary">
          <table id="dataTables1" class="table table-bordered table-striped table-hover">
            ...
            $query = mysqli_query($mysqli, "SELECT * FROM services ORDER BY services_id DESC")
            ...
            while ($data = mysqli_fetch_assoc($query)) { 
              ...
              echo "<tr>
                      ...
                      <td width='200' class='center'>
                        <img src='../images/servicios/$data[image]' height='150'>
                      </td>
                      <td class='center' width='80'>
                        <div>
                          <a data-toggle='tooltip' data-placement='top' title='Editar' style='margin-right:5px' class='btn btn-primary btn-sm' href='?module=form_services&form=edit&id=$data[services_id]'>
                              <i style='color:#fff' class='glyphicon glyphicon-edit'></i>
                          </a>";
            ?>
                          <a data-toggle="tooltip" data-placement="top" title="Eliminar" class="btn btn-danger btn-sm" href="modules/services/proses.php?act=delete&id=<?php echo $data['services_id'];?>" onclick="return confirm('Estas seguro que quieres eliminar <?php echo $data['title']; ?> ?');">
